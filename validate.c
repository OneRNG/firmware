...
//		b) encrypt a 1 block nonce of random data 'R' (16 bytes) -> 'ER'
//		c) encrypt (chained) 1 blocks of token 'T' (16 bytes) -> 'ET'
//		d) encrypt (chained) the final 32:95 bytes of the region 'S' (64 bytes) -> 'ES'
//		e) offer the resulting 96 bytes from the encryption to the host 'ER'+'ET'+'ES'
//	
//	6) user presents bytes 0:15 of the user data 'ID' and bytes 0:31 of the encrypted data 'ER'+'ET' to OneRNG's web page
//
//	7) OneRNG's server:
...
0077$:
        ...
        orl     _ENCCS, #1      	//      ENCCS |= 0x01;                  // start
        mov     r0, #16        		//      for (i=0; i<16; i++) {
0004$:        	mov	r1, #16		//            rand = 0
		clr	a		//	      for (j = 0; j < 16; j++) {
0005$:			mov	c, p1.7	//			rand = (rand<<1)|p1.7
			rlc	a
			jnc	0015$	//			rand = (rand&0xff)^(rand>>8)
				xrl	a, #1
0015$:					//	      }
                	djnz    r1, 0005$
                mov     _ENCDI, a	//	      ENCDI = rand
0011$:                				//	}
                djnz    r0, 0004$

        mov     r0, #255		//      wait_us(1);
0111$:  	djnz    r0, 0111$
